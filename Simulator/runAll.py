...
def heldMatch(map_file_path : str, names_of_teams : list):
    logger = JSONlogger()
    ...
    with open(f"./Match/log-Map {map_file_path} - Teams {'_'.join(names_of_teams)}.txt", "w") as log:
...
def main():
    ...
    names_of_teams = list()
    number_of_teams = int()
    with open("./Match/Configuration_4.TXT") as inputFile: 
    ...
    for map_file_path in maps_list:
        heldMatch(map_file_path, names_of_teams)
