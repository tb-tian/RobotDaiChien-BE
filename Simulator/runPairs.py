...
def heldMatch(map_file_path : str, firstTeamNaem : str, secondTeamName : str):
    names_of_teams = [firstTeamNaem, secondTeamName]
    logger = JSONlogger()
    board, frequency = readGameBeforeStarting(map_file_path + ".TXT")
    ...
    while not stopGame:
        ...
        logger.addNewEvent(turn, listOfPlayers, frequency, stopGame, board, lastPositions, initialBoard)
    ...
    #logger.writeFile(f"./Match/LogOfMatchOfTeams[{'_'.join(names_of_teams)}].json")
    logger.writeFile(f"./Match/{map_file_path}_{'_'.join(names_of_teams)}.json")

    with open(f"./Match/log-Map {map_file_path} - FirstTeam {names_of_teams[0]}- SecondTeam {names_of_teams[1]}.txt", "w") as log:
...
def main():
    ...
    names_of_teams = list()
    number_of_teams = int()
    with open("./Match/Configuration.TXT") as inputFile: 
        number_of_maps = int(inputFile.readline()[:-1])
        maps_list = [inputFile.readline().strip("\n") for i in range(number_of_maps)]
        number_of_teams = int(inputFile.readline()[:-1])
        names_of_teams = [inputFile.readline().strip("\n") for i in range(number_of_teams)]
    for map_file_path in maps_list:
        for i in range(number_of_teams):
            for j in range(i + 1, number_of_teams):
                print(f"Match: {names_of_teams[i]} vs {names_of_teams[j]} in map {map_file_path}.txt")
                heldMatch(map_file_path, names_of_teams[i], names_of_teams[j])
