...
//     // Try center first, ensure it's valid (empty and not going to be sealed)
//     if (gs.is_within_bounds(center_r, center_c) &&
//         gs.grid[center_r][center_c] == EMPTY_CELL &&
//         gs.is_valid_for_move(center_r, center_c, false))
//     {
//         return {center_r, center_c};
//     }

//     // Fallback: Iterate and find the first valid empty cell (e.g. row by row)
//     for (int r = 0; r < gs.M; ++r)
//     {
//         for (int c = 0; c < gs.N; ++c)
//         {
//             if (gs.grid[r][c] == EMPTY_CELL && gs.is_valid_for_move(r, c, false))
//             {
//                 return {r, c};
//             }
//         }
...
//     // Should always find one as per problem statement (unless map is full of obstacles/sealed cells)
//     // If no valid spot is found (highly unlikely for T=0), this is a problem. Default to center.
